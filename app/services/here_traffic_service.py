...
# HERE reports congestion/miscellaneous as "incidents" — those flood the table.
_SKIP_INCIDENT_TYPES = frozenset({"congestion", "miscellaneous"})


def parse_incident(raw: dict, location_name: str) -> Optional[dict]:
    """
    Convert a HERE incident Feature to internal incident format.
    Returns None when coordinates are missing or the type is not a real event.
    """
    props = raw.get("properties", {})
    geom  = raw.get("geometry", {})

    raw_type = props.get("incidentType", "miscellaneous").lower()
    if raw_type in _SKIP_INCIDENT_TYPES:
        return None

    mapped = _INCIDENT_TYPE_MAP.get(raw_type)
    if not mapped:
        return None
    inc_type, severity = mapped
